...
public final class Invoice {
	...
	public void extractLineItems(TimeCard timeCard) {
		...
		*/
		List<ConsultantTime> billableList = timeCard.getBillableHoursForClient(client.getName());
		billableList.stream().filter(ct->ct.getDate().getYear() == startDate.getYear() &&
				ct.getDate().getMonth() == startDate.getMonth())
		.forEach(ct->addLineItem(new InvoiceLineItem(ct.getDate(),timeCard.getConsultant(),ct.getSkill(),ct.getHours())));
		//.forEach(tc->addLineItem(new InvoiceLineItem(tc.getDate(),);
